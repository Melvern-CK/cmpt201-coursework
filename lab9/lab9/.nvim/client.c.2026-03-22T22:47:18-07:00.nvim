...
port number). Server address is 127.0.0.1 and port number is 8000

2. Is it UDP or TCP? How do you know?
  TCP because socket is created with SOCK_STREAM

3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? The clients gets the data from the user
input (most likely keyboard in this case). We can tell from read(STDIN_FILENO,
buf, BUF_SIZE).

4. How does the client program end? How can you tell that in the code?
  The client ends when it is out of the read() loop, it then calls close(sfd)
which closes socket and exits using exit(EXIT_SUCCESS)
